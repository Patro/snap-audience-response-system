...
RSpec.describe 'Questions API', type: :request do
  ...
  describe 'POST /questions' do
    ...
    context 'given data object with valid type' do
      let(:data) { data_with_relationship.merge(type: 'MultipleChoiceQuestion') }
    ...
    context 'given data object with missing type' do
      let(:data) { data_with_relationship.except(:type) }
    ...
    context 'given data object with non existing type' do
      let(:data) { data_with_relationship.merge(type: :non_existing_type) }
    ...
    context 'given data object with text' do
      let(:data) { data_with_relationship.merge(text: 'My Question') }
    ...
    context 'given data object with missing text' do
      let(:data) { data_with_relationship.except(:attributes) }
    ...
    describe 'interactive session' do
      ...
      context 'given interactive session as relationship only' do
        let(:data) { data_with_relationship }

        include_examples 'create resource', model_class: Question
      end

      context 'given interactive session as query param only' do
      ...
      context 'given interactive session as relationship and query param' do
        let(:data) { data_with_relationship }
        let(:query_params) { "interactive_session_id=#{interactive_session.id}" }
      ...
      context 'given different interactive session as relationship and query param' do
        let(:other) { create(:interactive_session) }
        let(:data) { data_with_relationship }
        let(:query_params) { "interactive_session_id=#{other.id}" }
